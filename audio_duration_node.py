...
class DVA_Audio_Duration_Calculator:
    ...
    def calculate_audio_duration(self, audio, calculation_mode="auto", time_precision=3,
        ...
        try:
            ...
            audio_path = self._get_audio_path(audio)
            logger.info(f"Получен путь к аудио: {audio_path}")
            ...
            logger.info(f"Исходная длительность: {duration} секунд")
    ...
    def _get_audio_path(self, audio_input):
        ...
        logger.info(f"Тип входных данных: {type(audio_input)}")
        ...
        # 2. Если это словарь (стандартный формат ComfyUI)
            ...
            # Если есть данные в виде тензора или байтов
            if 'audio_data' in audio_input or 'data' in audio_input:
                # Сохраняем временный файл
                return self._save_temp_audio(audio_input)
        
        # 3. Если это объект с атрибутами
        elif hasattr(audio_input, '__dict__'):
            logger.info(f"Входные данные - объект. Атрибуты: {dir(audio_input)}")
            
            # Пробуем получить путь через атрибуты
            for attr_name in ['file_path', 'filename', 'path', 'audio_path']:
                if hasattr(audio_input, attr_name):
                    path = getattr(audio_input, attr_name)
                    if path and os.path.exists(path):
                        return path
        
        # 4. Если это список (возможно, из Load Audio ноды)
        elif isinstance(audio_input, list) and len(audio_input) > 0:
            logger.info(f"Входные данные - список. Первый элемент: {audio_input[0]}")
            # Рекурсивно обрабатываем первый элемент
            return self._get_audio_path(audio_input[0])
        
        logger.warning("Не удалось определить путь к аудио файлу")
        return None
    ...
    def _save_temp_audio(self, audio_data):
        ...
        try:
            ...
            if isinstance(data, bytes):
                ...
                sf.write(temp_path, data, 44100)  # Дефолтная частота
            elif torch.is_tensor(data):
                # Конвертируем тензор в numpy и сохраняем
                numpy_data = data.cpu().numpy()
                import soundfile as sf
                sf.write(temp_path, numpy_data, 44100)
            else:
                return None
...
class DVA_Audio_Metadata_Extractor:
    ...
    def extract_audio_metadata(self, audio, extract_format=True, extract_technical=True, extract_tags=False):
        ...
        try:
            # Используем тот же метод получения пути, что и в калькуляторе
